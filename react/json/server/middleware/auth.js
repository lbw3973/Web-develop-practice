...
const tokenRefresh = (req, res, next) => {
  ...
  if (postData.refreshToken && postData.refreshToken in refreshList) {
    const decoded = jwt.verify(postData.refreshToken, process.env.SECRET_RTOKEN);

    const token = generateAccessToken(decoded.id, decoded.username, decoded.email);
    const refreshToken = generateRefreshToken(decoded.id, decoded.username, decoded.email);
...
function generateAccessToken(id, username, email) {
  return jwt.sign({ id: id, username: username, email: email }, process.env.SECRET_TOKEN, {
    expiresIn: "1h",
  });
}
